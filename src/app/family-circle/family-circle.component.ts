...
private startPersonID: string = "SheilaMaryPreece1953"; 
private startPerson: Person;
private currentPerson: Person; private previousPerson2: Person;private previousPerson3: Person;private previousPerson4: Person;
...
//#endregion
    ...
    this.navigationSubscription = this.router.events.subscribe((e: any) => {
      ...
      if (e instanceof NavigationEnd) {
        this.route.paramMap.subscribe(route => {
          this.startPersonID = route.get('personID');
        });
          ...
          this.startPerson = this.family.find(person => person.personID == this.startPersonID);
          let startSpouse = this.family.find(person => person.personID == this.startPerson.spouseID)
          ...
          this.ctx.fillText("The Family Tree of " + this.startPerson.name + " " + this.startPerson.surname + " and " + startSpouse.name + " " + startSpouse.surname, this.drawWidth/2, 40);
  ...
  private DrawGenerations() {
    for (let i = 0; i < 2; i++) {
      this.startPerson = this.family.find(person => person.personID == this.startPersonID);
      this.xPosn = this.drawWidth /2;
      this.yPosn = this.drawHeight/2 + this.upDown * this.ySeparation;
      this.PersonDisplay(this.startPerson,this.xPosn,this.yPosn);
      for (let j = 2; j < 5; j += 2) {
        this.xPosn = this.drawWidth * j /6;
        this.yPosn = this.drawHeight/2 + this.upDown * this.ySeparation;
        if (j == 2) {
          this.currentPerson = this.family.find(person => person.personID == this.startPerson.motherID);
        }
        else {
          this.currentPerson = this.family.find(person => person.personID == this.startPerson.fatherID);
        }
      ...
      this.count3 = 5;
      this.count4 = 9;
      this.startPersonID = this.startPerson.spouseID;
    }
  }
